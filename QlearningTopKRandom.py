"""
文件名：QlearningTopKRandom.py
方法名：Qlearning-TopKRandom

功能：单Q表 + LSTM 预测框架下的 Top-K Random 探索。
...
class QlearningTopKRandom:
    ...
    def _top_k_random_action(self, q_values):
        shuffled_q_values = q_values.reindex(np.random.permutation(q_values.index))
        k = min(self.top_k, len(self.actions))
        top_k_actions = list(shuffled_q_values.nlargest(k).index)
        return np.random.choice(top_k_actions)
    ...
    def choose_action_lstm(self, observation, prediction):
        ...
        if np.random.uniform() < self.epsilon:
            ...
            action = self._top_k_random_action(q_values)
