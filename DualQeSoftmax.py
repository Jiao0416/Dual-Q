"""
文件名：DualQ-εSoftmax.py
功能：在双Q表 + LSTM 后验融合框架下，将探索阶段改为 ε-Softmax 探索。

核心对比点：
1. 利用阶段：仍然根据 LSTM 加权融合后的双Q值选择最大动作；
2. 探索阶段：不再使用均匀随机或 LSTM 空闲概率随机，而是根据 fused Q 值做 Softmax 加权采样。

该文件用于和 DualQ_plus.py 中的 LSTM-directed exploration 做控制变量对比。
"""
...
class DualQESoftmax:
    ...
    def _softmax_action(self, q_values):
        """根据 fused Q 值进行 Softmax 加权采样"""
        temperature = max(self.temperature, 1e-8)
        values = q_values.to_numpy(dtype=np.float64)
        ...
        if np.sum(exp_values) == 0 or np.any(np.isnan(exp_values)):
            prob_distribution = np.ones(len(self.actions)) / len(self.actions)
        else:
    ...
    def choose_action_lstm(self, observation, prediction, num_channel):
        ...
        - 否则：探索，根据 softmax(fused Q / temperature) 加权随机选择动作。
        """
        ...
        if np.random.uniform() < self.epsilon:
            ...
            action = self._softmax_action(weighted_q_values)
    ...
    def choose_action(self, observation):
        ...
        if np.random.uniform() < self.epsilon:
            ...
            action = self._softmax_action(weighted_q_values.astype(np.float64))
